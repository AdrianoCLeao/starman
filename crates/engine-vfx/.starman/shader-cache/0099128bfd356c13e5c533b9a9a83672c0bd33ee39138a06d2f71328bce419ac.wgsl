...
fn over_life_speed(t: f32) -> f32 {
    let x = lut_position(t);
    let i = u32(x.x);
    return mix(lane(emitter.speed_lut[i / 4u], i % 4u), lane(emitter.speed_lut[(i + 1u) / 4u], (i + 1u) % 4u), x.y);
}
...
fn update(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (emitter.noise.w > 0.5) {
        ...
        velocity += curl_noise(q) * emitter.noise.x * dt;
    }
    var speed_scale = 1.0;
    if (emitter.accel.w > 0.5) {
        speed_scale = over_life_speed(life);
    }
    var next = p.position_age.xyz + velocity * speed_scale * dt;

    if (emitter.collision.x > 0.5 && scene.camera.w > 0.5) {
